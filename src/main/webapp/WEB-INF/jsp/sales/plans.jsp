...
<script type="text/javascript">
	...
	$(function(){
		...
		$('.search').click(function(){
			var addr = $("select[name=city] option:selected").val();
			
			var url = "plan/findChanceByUserNameAndAddr/1";
			var data = {
					addr:addr
			}
			console.log(data);
		...
		$('.layui-icon-loading').click(function(){
			...
			$('.layui-body').load('plan/toPlanAdd/'+chcId);  
		})
		
		$('.layui-icon-fonts-code').click(function(){
			chcId = $(this).attr("value");
			$('.layui-body').load('plan/toPlanEdit/'+chcId);  
		})
		
		$('.layui-icon-face-smile').click(function(){
			chcId = $(this).attr("value");
			$('.layui-body').load('plan/toPlanDetail/' + chcId);  
		})
	})
</script>